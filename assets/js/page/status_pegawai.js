        ...
        success:function(resp){
            ...
            if(res.is_error){
                ...
                $.each(res.data,function(k,v){
                    ...
                    html += "<td>" + v['nama'] + "</td>";
                    html += "<td>";
                    html += "<a href='javascript:void(0);' onclick='move_up(" + v['order'] + ")' class='btn btn-sm btn-light'><span class='fa fa-angle-up'></span></a> ";
                    html += "<a href='javascript:void(0);' onclick='move_down(" + v['order'] + ")' class='btn btn-sm btn-light'><span class='fa fa-angle-down'></span></a>";
                    html += "</td>";
                    html += "<td>";
                    html += "<a href='/status_pegawai/edit/" + v['id'] + "' class='btn btn-sm btn-light'><span class='fa fa-edit'></span></a> ";
...
function move_up(order){
    ...
    $.ajax({
        type:'post',
        url:'/ajax/status_pegawai/move_up',
        data:{order:order},
        success:function(resp){
            $("#listdata").loading("stop");
            var res = JSON.parse(resp);
...
function move_down(order){
    ...
    $.ajax({
        type:'post',
        url:'/ajax/status_pegawai/move_down',
        data:{order:order},
        success:function(resp){
            $("#listdata").loading("stop");
            var res = JSON.parse(resp);
